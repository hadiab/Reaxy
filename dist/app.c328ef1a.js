...
var getModelName = function getModelName(path) {
  return path.trim().split('/')[0];
};

var mapStore = function mapStore(models) {
  var initialState = {};
  var actions = {}; // Map models and create initial state object & actions object

  for (var model in models) {
    initialState[model] = models[model].state;

    for (var action in models[model].actions) {
      actions["".concat(model, "/").concat(action)] = models[model].actions[action];
    }
  ...
  return {
    ...
    actions: function actions(state, action) {
